...
| session.redis.max.life.time.sec |  Maximum lifetime of a session. [It will be the lowest value between it and the one configured in the keycloak in `SSO Session Max` in the `Tokens` tab of the Realm settings or the values configured on the client in Advanced Settings.] |86400 | number (seconds) | BS_SESSION_REDIS_MAX_LIFE_TIME_SEC
